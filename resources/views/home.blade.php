    ...
    <div class="row blog-entries">
        <div class="col-md-8 main-content">
            <div class="row cards-row">
                ...
                <div class="col-md-6">
                    <a href="/post/{{$post->slug}}" class="blog-entry ">
                        ...
                        <div class="blog-content-body">
                            <div class="post-meta">
                                <span class="author"><img src="{{$post->user->profile_photo}}" alt="Colorlib">
                                    {{$post->user->name}}</span>
                ...
                @elseif (request('tag'))
                <p class="text-danger" style="margin: auto">{{request('tag')}} -ამ თეგზე პოსტები არ არსებობს</p>
                @endif
            ...
            <div class="row mt-5 mb-5" style="margin: auto">
                <div class="col-md-12 pag">                       
                    @if (request('title'))
                    {!! $posts->appends(['title' => request('title')])->render() !!}
                    @elseif (request('tag'))
                    {!! $posts->appends(['tag' => request('tag')])->render() !!}
                    @else
                    {!! $posts->render() !!}
                    @endif
                </div>
            ...
            <div class="sidebar-box search-form-wrap">
                <form action="{{route('post.index',['title'=>request('title')])}}" method="GET" class="search-form">
                    <div class="form-group">
                        <form action="">
                        <input type="text" name="title" class="form-control" id="s" placeholder="Search Blog With Title">
